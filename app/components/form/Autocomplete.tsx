      ...
      <Combobox as="div" {...getInputProps()}>
        ...
        <div className="relative my-1">
          <Combobox.Input<T["value"]>
            ...
            displayValue={(value) => options.find((o) => o.value === value)?.label || ""}
            onChange={(event) => setQuery(event.target.value)}
          />
          <Combobox.Button className="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none">
            <ChevronUpDownIcon aria-hidden="true" className="h-5 w-5 text-gray-400" />
          </Combobox.Button>
          {filteredOptions.length > 0 && <VirtualizedOptions options={filteredOptions} />}
        </div>
        {description && (
          <p
        ...
        <p className={clsx("text-sm text-red-600", error ? "block" : "hidden")}>{error}</p>
      </Combobox>
    </>
  )
}
